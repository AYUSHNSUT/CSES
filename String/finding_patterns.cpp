...
void solve(){
    ...
    cin >> s;
    ll n = s.size();
    preprocess(primePower1,A,p1);
    unordered_map <ll,ll> done;
    hashedString1 = getHashVector(s,p1,A);
    vl ans;
    for(int length = 1;length<=n;length++){
        ...
        bool thisAns = true;
        ll hashedSub = substringHash(hashedString1,0,length-1,p1,primePower1);
        // /DEBUG(length);
        for(int j = length; thisAns && j + length<=s.size();j+=length){
            // DEBUG(j);
            // DEBUG(j+length-1);
            thisAns &= (hashedSub == substringHash(hashedString1,j,j+length-1,p1,primePower1));
        }
        ...
        if(n%length){
            ...
            // DEBUG(rem);
            // DEBUG(thisAns);
            thisAns&= (substringHash(hashedString1,0,rem-1,p1,primePower1) == substringHash(hashedString1,n-rem,n-1,p1,primePower1));
        }

        if(thisAns){
          //  cout << length << " ";
          ans.pb(length);
            for(ll z = length;z<=n;z+=length){
                done[z] = 1;
            }
        }
    }
